...
### 1. ⚡ 控制面板 (`console-view`)
* 🚀 **核心定位**：整个 Nexora Agent 软件的核心总控台与生命周期管理者。
* 🔘 **主要控件与界面元素**：
...
* ⚙️ **底层机制**：点击启动后，主进程执行 `gateway-boot-harden.js`，自动擦除异常残留锁并挂载便携 `.node-sandbox` 环境。
...
* 🔘 **主要控件与界面元素**：
  ...
  * **微信「扫码绑定」按键**：点击一键生成高清 Base64 二维码弹窗，微信手机扫码即可完成授权。
  * **飞书 / QQBot 凭证配置**：配置飞书 App ID / App Secret、QQBot AppID / Token 等官方凭证。
...
* 🔘 **主要控件与界面元素**：
  ...
  * **API Key & Base URL 填报框**：凭证密文存储于本地配置文件，绝对不出本机。
  * **「测试连接」探针按钮**：向对应 API 发送轻量探针包，实时反馈响应延迟（ms）与可用状态。
  * **视觉模型自动对齐开关 (Vision Model Auto-Align)**：自动将包含图片理解能力的模型标记为 `image` 兼容，防止传图时报错。
...
* 🔘 **主要控件与界面元素**：
  ...
  * **「新建角色」按键**：允许用户自定义角色的名称、图标、System Prompt 文本及温度值（Temperature）。
  * **设为默认角色按键**：一键将特定角色指定为全局默认接管人格。
...
* 🔘 **主要控件与界面元素**：
  ...
  * **富媒体气泡区**：完美支持 Markdown 语法、代码高亮一键复制、LaTeX 公式、图片/视频/音频直接预览播放。
  * **「新建会话」按键**：重置当前上下文，建立独立的新对话隔离区。
  * **「清空历史」按键**：擦除当前会话在本地内存中的临时缓存。
  * **无缝滚存提示条**：触发滚存时显示：“系统已自动完成上下文滚存与摘要提取”。
...
* 🔘 **主要控件与界面元素**：
  ...
  * **主题与样式自动同步**：系统自动将外层的赛博暗黑主题样式注入 Webview 内，保持视觉高度一体化。
...
* 🔘 **主要控件与界面元素**：
  ...
  * **首字延迟 (TTFT) 柱状图**：对比不同模型厂商的响应速度与首字输出耗时。
  * **调用成功率分布图**：展示 API 调用的成功与失败比例饼图。
...
* 🔘 **主要控件与技能项**：
  ...
  * 🖱️ **物理桌面操控 (Desktop Control)**：通过 user32.dll 事件驱动模拟真实键盘与鼠标点击。
...
* 🔘 **主要控件与界面元素**：
  ...
  * **关键字检索框**：支持根据关键词对历史会话标题及聊天正文进行全局模糊匹配搜索。
  * **「导出 Markdown」按键**：将选中的会话一键导出为纯文本 `.txt` 或 Markdown `.md` 文件。
  * **批量清理按钮**：支持清理过期或失效的废弃会话记录。
...
* 🔘 **主要控件与界面元素**：
  ...
  * **快捷诊断脚本集**：内置一键检查端口占用、查看日志文件、清理沙箱缓存的快捷按键。
...
* 🔘 **主要控件与界面元素**：
  ...
  * **级别筛选按钮**：支持按 `ALL`、`INFO`、`WARN`、`ERROR` 进行快速过滤。
  * **日志导出与清除**：支持一键清空显示或将日志导出保存。
...
* 🔘 **主要控件与界面元素**：
  ...
  * **开机静默自启开关**：设置软件随系统开机自动后台运行。
  * **「Doctor 完整性诊断」按键**：一键扫描系统依赖、端口占用与沙箱健康状态。
